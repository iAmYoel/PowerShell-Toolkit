function New-IntuneWin32AppRequirementRuleScript {
    ...
    .PARAMETER ScriptContentAsBase64
        Specify the script content as base64 string.
    ...
    param(
        ...
        [parameter(Mandatory = $true, ParameterSetName = "String", HelpMessage = "Specify the script content as base64 string.")]
        [parameter(Mandatory = $true, ParameterSetName = "Integer")]
        [parameter(Mandatory = $true, ParameterSetName = "Boolean")]
        [parameter(Mandatory = $true, ParameterSetName = "DateTime")]
        [parameter(Mandatory = $true, ParameterSetName = "Float")]
        [parameter(Mandatory = $true, ParameterSetName = "Version")]
        [parameter(Mandatory = $true, ParameterSetName = "ScriptContentAsBase64")]
        [ValidateScript({
            try { $null=[convert]::frombase64string($item); $true } catch { $false }
        })]
        [string]$ScriptContentAsBase64,
    ...
    Process {
        ...
        if ($ScriptFile -and $ScriptContentAsBase64){
            Write-Warning "Please use only one parameter between ScriptFile and ScriptContentAsBase64."
            Exit 1
        } elseif ($ScriptContentAsBase64) {
            $ScriptContent = $ScriptContentAsBase64
        } elseif ($ScriptFile) {
