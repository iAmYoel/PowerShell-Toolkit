function Test-IntuneWin32AppAssignment {
    ...
    param(
        ...
        [parameter(Mandatory = $true, HelpMessage = "Specify the group mode of the assignment, GroupInclude or GroupExclude.")]
        [ValidateNotNullOrEmpty()]
                        ...
                        foreach ($Win32AppAssignment in $Win32AppAssignments.value) {
                            if ($Win32AppAssignment.target.'@odata.type' -match "groupAssignmentTarget") {
                                if ($Win32AppAssignment.target.groupId -like $GroupID) {
                                    if ($GroupMode -notlike "GroupExclude" -and $Intent -notlike "uninstall") {
