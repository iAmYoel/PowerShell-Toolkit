function Test-IntuneWin32AppAssignment {
    ...
    param(
        ...
        [parameter(Mandatory = $false, HelpMessage = "Specify the intent of the assignment, required, available or uninstall.")]
        [ValidateNotNullOrEmpty()]
        [ValidateSet("Required", "Available", "Uninstall")]
        [string]$Intent,

        [parameter(Mandatory = $false, HelpMessage = "Specify the group mode of the assignment, GroupInclude or GroupExclude.")]
        [ValidateNotNullOrEmpty()]
        [ValidateSet("GroupInclude", "GroupExclude")]
        [string]$GroupMode
    )
    Process {
                        ...
                        foreach ($Win32AppAssignment in $Win32AppAssignments.value) {
                            if ($Win32AppAssignment.target.'@odata.type' -match "groupAssignmentTarget") {
                                if ($Win32AppAssignment.target.groupId -like $GroupID) {
                                    if ($Win32AppAssignment.intent -like $Intent) {
                                        Write-Warning -Message "Win32 app assignment with id '$($Win32AppAssignment.id)' of target type '$($Target)' and GroupID '$($Win32AppAssignment.target.groupId)' with the intent '$($Win32AppAssignment.intent)' already exists, duplicate assignments of this type is not permitted"
                                        $DuplicateAssignmentDetected = $true
                                    }elseif (($Win32AppAssignment.target.'@odata.type' -match "graph\.groupAssignmentTarget") -AND ($GroupMode -like "GroupInclude")){
                                        Write-Warning -Message "Win32 app assignment with id '$($Win32AppAssignment.id)' of target type '$($Target)' and GroupID '$($Win32AppAssignment.target.groupId)' with the intent '$($Win32AppAssignment.intent)' and group mode 'Include' already exists, duplicate assignments of this type is not permitted"
                                        $DuplicateAssignmentDetected = $true
                                    }
                                }
                            }
