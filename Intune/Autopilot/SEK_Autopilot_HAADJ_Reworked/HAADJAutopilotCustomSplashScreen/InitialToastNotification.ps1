...
Param(
    ...
    [Parameter(Mandatory=$False)]
    [String]$Message = "Your device is being setup. You will get a notification again when the device is ready.",
...
if ((Get-Item $user.PSPath).Property -contains "LoggedOnDisplayName" -eq $true) {
    $user = $user.LoggedOnDisplayName
} else {
    $user = $user.LoggedOnUser | %{$_.Split('\')[1]}
}

$WelcomeText = "Hi $user! "
$Text = $WelcomeText + $Message
...
# Run FinalToastNotification_Trigger.ps1
Start-Process -FilePath "Powershell.exe" -ArgumentList "-ExecutionPolicy Bypass -WindowStyle Hidden -NoLogo -File `"C:\ProgramData\AADHybridLockOOBE\FinalToastNotification_Trigger.ps1`""
