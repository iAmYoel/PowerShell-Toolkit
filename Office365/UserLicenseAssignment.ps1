...
function Get-LicensedGroup{
    ...
    Foreach ($item in $Name) {
        $AllGroups = (Get-MsolGroup -SearchString $item) | where{$_.Licenses}
    }

    Foreach ($item in $ObjectId){
        $AllGroups = (Get-MsolGroup -ObjectId $item)| where{$_.Licenses}
    }

    if ((!$ObjectId) -and (!$Name)) {
        $AllGroups = (Get-MsolGroup -All) | where{$_.Licenses}
    }
...
Foreach ($item in $AllGroups) {
    ...
    $groupLicenses = $item.Licenses | Select -ExpandProperty SkuPartNumber
    $totalCount = 0;
...
function Get-MsolUserDirectAssignedLicense {
