...
$Modules = @("ActiveDirectory","DnsServer","GroupPolicy","RemoteDesktop")
foreach ($Module in $Modules) {
    ...
    IF(!$CurrentModule){
        ...
        IF((@("ActiveDirectory","DnsServer","GroupPolicy") -contains $Module) -AND ($env:COMPUTERNAME -notlike $DomainController)){
            Try{
...
IF(!(Get-ADGroup -Filter "Name -like '$RDSAccessGroup'")){
    Try{
        ...
        (Get-ADGroup $NewADGroup -Properties CanonicalName).CanonicalName
...
$InstallMediaPath = "$FSLogixPath\FSLogixAppsSetup.exe"
$LocalInstallPath = "C:\Windows\Temp\FSLogixAppsSetup.exe"

IF(Test-Path $InstallMediaPath -PathType Leaf){
    ...
    $OUPath = Get-ADOrganizationalUnit -Filter * -Properties CanonicalName | select CanonicalName | Out-GridView -Title "Choose an OU for FSLogix AD groups" -PassThru

    Foreach($Server in $DefinedRDSH){

        Clear-Variable InstallResult,RemoteInstallPath -ErrorAction SilentlyContinue
        $RemoteInstallPath = "\\$($Server.Name)\$($LocalInstallPath -replace ":","$")"

        Try{
            Copy-Item $InstallMediaPath $RemoteInstallPath -Force -ErrorAction Stop | Out-Null
    ...
    Write-LogEntry -Severity ERROR -Message "The FSLogix source path '$InstallMediaPath' could not be validated"
