...
"`r" | Out-File $loginfo -append
    ...
    function set-account-memberof
    {
        ...
        if($o365 -eq "E1")
        {
            $o365group = "SG Office365 E1"
            $o365member = Get-ADPrincipalGroupMembership $alias | Where {($_.Name -like 'SG Office365*')}
            if(!$o365member)
            {
            ...
            {
                Get-ADGroup $o365member | Remove-ADGroupMember -Members $alias -Confirm:$false
                Get-ADGroup $o365group | Add-ADGroupMember -Members $alias
            }
        }
        if($o365 -eq "E3")
        {
            $o365group = "SG Office365 E3"
            $o365member = Get-ADPrincipalGroupMembership $alias | Where {($_.Name -like 'SG Office365*')}
            if(!$o365member)
            {
                Get-ADGroup $o365group | Add-ADGroupMember -Members $alias
        ...
        if($o365 -eq "Ingen licens" -or $o365 -eq "Underkonsult" -or $o365 -eq "UK")
        {
            $o365member = Get-ADPrincipalGroupMembership $alias | Where {($_.Name -like 'SG Office365*')}
            if(!$o365member)
            {
