...
# Check SearchPath
IF (!(Test-Path -PSPath "\\?\$($Path -replace "^\\","UNC\")" -PathType Container)){
...
if (!($tempFolderList = Get-Item $tempFolderPathFile*.txt | sort CreationTime | select -Last 1)){
    Write-Output "Fetching all folders..." | Out-File -FilePath "$PSScriptRoot\Get-FolderACL_Progress.log" -Encoding UTF8 -Force
    $i=0

    # Get permissions from target folder
    $ParentFolderPath = Get-Item -PSPath "\\?\$realPath" -Force -ErrorAction SilentlyContinue -ErrorVariable +Errorlist
    IF($Recurse){

        # Get permissions from child folders
        Get-ChildItem -Directory -PSPath "\\?\$realPath" -Recurse -Force -OutVariable ChildFoldersPath -ErrorAction SilentlyContinue -ErrorVariable +ErrorList | ForEach-Object{
            ...
            if (($startTime.Elapsed -ge $timespan) -or ($i -eq 0)) {
                ...
                Write-Output "$i folders fetched..." | Out-File -FilePath "$PSScriptRoot\Get-FolderACL_Progress.log" -Encoding UTF8 -Force
        ...
        # Get permissions from child folders
        Get-ChildItem -Directory -PSPath "\\?\$realPath" -Force -OutVariable ChildFoldersPath -ErrorAction SilentlyContinue -ErrorVariable +ErrorList | ForEach-Object{
    ...
    Write-Output "Folders fetched!" | Out-File -FilePath "$PSScriptRoot\Get-FolderACL_Progress.log" -Encoding UTF8 -Force
...
<#     # Error handling
    ...
    $FolderPath | Out-File -FilePath "$tempFolderPathFile$Date.txt" -Encoding UTF8 -Force -pas
    $tempFolderList = Get-Item "$tempFolderPathFile$Date.txt" -Force -ErrorVariable +ErrorList
...
$i=0
$tempPermListFilePrefix = "$PSScriptRoot\FolderACL_InProgress_"
if ($tempPermList = (Get-Item $tempPermListFile*.csv -Force | sort CreationTime | select -Last 1).FullName) {
    try {
        $i = $i + (Import-Csv -Delimiter ";" -Path $tempPermList -Encoding UTF8 -ErrorAction Stop).Count
    }
    catch {
        Write-Warning "Failed to import existing temp report csv."
    }
} else {
    $tempPermList = "$tempPermListFilePrefix$date.csv"
}
#$Report = @()
#$ExportCap = 20000
#$AddToCap = $ExportCap
...
Write-Output "Processing folders..." | Out-File -FilePath "$PSScriptRoot\Get-FolderACL_Progress.log" -Encoding UTF8 -Force
...
Foreach ($Folder in ($FolderPath | select -Skip $i)) {
    ...
    if (($startTime.Elapsed -ge $timespan) -or ($i -eq 1)) {
        ...
        Write-Output "$i of $tot folder processed..." | Out-File -FilePath "$PSScriptRoot\Get-FolderACL_Progress.log" -Encoding UTF8 -Force
        Write-Progress @Progress
        $startTime.Restart()
    }
    #endregion
    ...
    $ACL = Get-Acl -PSPath $Folder -ErrorVariable +ErrorList -ErrorAction Stop
    ...
    foreach ($Access in $ACL.Access){
        ...
        IF ($AccessRights){
            ...
            $Report = [PSCustomObject][Ordered]@{'FolderName'=$(($ACL.Path | Convert-Path) -replace "\\\\\?\\");'Identity'=$Access.IdentityReference;'Permissions'=$AccessRights;'Type'=$Access.AccessControlType;'AppliesTo'=$AccessAppliedTo;'PermissionInherited'=$Access.IsInherited;'FolderInheritance'=$FolderInheritance}
            Export-Csv -InputObject $Report -Path $tempPermList -Encoding UTF8 -Append -Delimiter ";" -NoTypeInformation -Force
...
IF ($ExportFormat -notlike "CSV") {
    Import-Csv -Delimiter ";" -Path $tempPermList -Encoding UTF8 -ErrorVariable +ErrorList | Export-Excel -Path $XLSXExportPath -WorksheetName "Report" -AutoSize -FreezeTopRow -BoldTopRow -TableName Report -ErrorVariable +ErrorList
}

IF ($ExportFormat -notlike "XLSX") {
    $newFile = Rename-Item -Path $tempPermList -NewName (Split-Path $CSVExportPath -Leaf) -PassThru -ErrorVariable +ErrorList
    $newFile = Move-Item -Path $newFile -Destination $CSVExportPath -Force -PassThru -ErrorVariable +ErrorList
} else {
    Remove-Item -Path $tempPermList -Force -Confirm:$false -ErrorVariable +ErrorList
}


Write-Output "Export done!" | Out-File -FilePath "$PSScriptRoot\Get-FolderACL_Progress.log" -Encoding UTF8 -Force
...
IF($NoErrorLog){
    ...
    Write-Output "Export successfully completed!" | Out-File -FilePath "$PSScriptRoot\Get-FolderACL_Progress.log" -Encoding UTF8 -Force
    ...
    # Set progress bar variables
    $i=0
    $tempErrorListFilePrefix = "$PSScriptRoot\FolderACL_ErrorLog_InProgress_"
    if ($tempErrorList = (Get-Item $tempErrorListFile*.csv -Force | sort CreationTime | select -Last 1).FullName) {
        try {
            $i = $i + (Import-Csv -Delimiter ";" -Path $tempErrorList -Encoding UTF8 -ErrorAction Stop).Count
        }
        catch {
            Write-Warning "Failed to import existing temp report csv."
        }
    } else {
        $tempErrorList = "$tempErrorListFilePrefix$date.csv"
    }
    $tot = $ErrorList.count
    $startTime.Restart()
    ...
    Foreach ($ErrorItem in $ErrorList){
        ...
        if (($startTime.Elapsed -ge $timespan) -or ($i -eq 1)) {
            ...
            $Progress = @{
                Id = 3
                Activity = "Step 3 of $($AllSteps): Exporting errors."
                CurrentOperation = $WriteTime
                Status = "Processing error $i of $tot : $status% Completed"
                PercentComplete = $percent
            }
        ...
        # Create hash table and add values.
        $ErrorLog = [PSCustomObject][Ordered]@{'Command'=$ErrorItem.CategoryInfo.Activity;'FolderName'=$ErrorItem.CategoryInfo.TargetName;'Category'=$ErrorItem.CategoryInfo.Category;'Reason'=$ErrorItem.CategoryInfo.Reason;'Message'=$ErrorItem.Exception.Message}

        Export-Csv -InputObject $ErrorLog -Path $tempErrorList -Encoding UTF8 -Append -Delimiter ";" -NoTypeInformation -Force
    ...
    if ($ErrorLog){
        ...
        Write-Output "Export completed with errors! Exporting error log to csv." | Out-File -FilePath "$PSScriptRoot\Get-FolderACL_Progress.log" -Encoding UTF8 -Force

        IF ($ExportFormat -notlike "CSV") {
            Import-Csv -Delimiter ";" -Path $tempErrorList -Encoding UTF8 -ErrorVariable +ErrorList | Export-Excel -Path $XLSXErrorExportPath -WorksheetName "Report" -AutoSize -FreezeTopRow -BoldTopRow -TableName Report -ErrorVariable +ErrorList
        }

        IF ($ExportFormat -notlike "XLSX") {
            $newErrorFile = Rename-Item -Path $tempErrorList -NewName (Split-Path $CSVErrorExportPath -Leaf) -PassThru -ErrorVariable +ErrorList
            $newErrorFile = Move-Item -Path $newErrorFile -Destination $CSVErrorExportPath -Force -PassThru -ErrorVariable +ErrorList
        } else {
            Remove-Item -Path $tempErrorList -Force -Confirm:$false
        }

    }ELSE{
        Write-Output "Export successfully completed with no errors!" | Out-File -FilePath "$PSScriptRoot\Get-FolderACL_Progress.log" -Encoding UTF8 -Force
    }
}
...
IF($ShowDuration){
    ...
    Write-Output "Script duration: $WriteTime" | Out-File -FilePath "$PSScriptRoot\Get-FolderACL_Progress.log" -Encoding UTF8 -Force
...
Remove-Item -Path "$PSScriptRoot\Get-FolderACL_Progress.log" -Force -Confirm:$false
