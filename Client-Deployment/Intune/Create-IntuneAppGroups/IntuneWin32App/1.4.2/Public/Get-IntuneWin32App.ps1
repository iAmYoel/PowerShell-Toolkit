        ...
        switch ($PSCmdlet.ParameterSetName) {
                ...
                if ($Win32MobileApps -ne $null) {
                    ...
                    if ($Win32MobileApps -ne $null) {
                        foreach ($Win32MobileApp in $Win32MobileApps) {
                            ...
                            if(($categories = Invoke-MSGraphOperation -Get -APIVersion "Beta" -Resource "deviceAppManagement/mobileApps/$($Win32MobileApp.id)/categories").'@odata.context'){
                                $categoriesValue = $categories.value
                            }else{
                                $categoriesValue = $categories
                            }
                            $Win32App | Add-Member -MemberType NoteProperty -Name categories -Value $categoriesValue
            ...
            default {
                ...
                if ($Win32MobileApps.Count -ge 1) {
                    $i=0
                    foreach ($Win32MobileApp in $Win32MobileApps) {
                        Write-Progress -Activity "Retrieve all Intune Win32 Apps" -Status "$i of $($Win32MobileApps.Count) apps retrieved" -PercentComplete ($i / $Win32MobileApps.Count * 100)
                        $Win32App = Invoke-MSGraphOperation -Get -APIVersion "Beta" -Resource "deviceAppManagement/mobileApps/$($Win32MobileApp.id)"
                        if(($categories = Invoke-MSGraphOperation -Get -APIVersion "Beta" -Resource "deviceAppManagement/mobileApps/$($Win32MobileApp.id)/categories").'@odata.context'){
                            $categoriesValue = $categories.value
                        }else{
