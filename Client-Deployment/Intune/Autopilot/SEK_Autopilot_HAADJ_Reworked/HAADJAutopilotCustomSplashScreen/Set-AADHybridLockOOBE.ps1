...
# Check if Windows license is Volume, if so change to OEM
Write-Log "Checking if Windows license is a Volume license"
$CheckLicense = Get-WmiObject -Query 'Select * from SoftwareLicensingProduct' | where {$_.Name -match "Windows" -and $_.LicenseStatus -eq 1}

If (($CheckLicense | measure).Count -eq 1 -and $CheckLicense.Description -match "Volume"){
  Write-Log "Windows license is a Volume license. Changing to OEM license."
  $OEMLicense = (Get-WmiObject -Query 'Select * from SoftwareLicensingService').OA3xOriginalProductKey
  slmgr //b /ipk $OEMLicense
}else {
  Write-Log "Windows license is not a Volume license"
}
