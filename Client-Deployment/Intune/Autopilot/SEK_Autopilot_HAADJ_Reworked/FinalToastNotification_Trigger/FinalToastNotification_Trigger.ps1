...
function Test-WindowsLicense {
  if ((Get-CimInstance SoftwareLicensingProduct -Filter "Name like 'Windows%'" | ? PartialProductKey).PartialProductKey -match "3V66T"){
    return "OK"
  } else {
    return "Not OK"
  }
...
do{
  ...
  $AppInstallStatus = ""
  $AppInstallStatus = Test-WindowsLicense
