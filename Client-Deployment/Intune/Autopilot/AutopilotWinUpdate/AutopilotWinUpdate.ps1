...
# Start logging
$LogPath = "C:\ProgramData\Microsoft\AutopilotWinUpdate"
New-Item $LogPath -ItemType Directory -Force
Start-Transcript "$LogPath\AutopilotWinUpdate.log"

# Create a tag file just so Intune knows this was installed
$regPath = "HKLM:\SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\AutopilotWinUpdate"
if (-not (Test-Path $regPath))
{
    New-Item -Path $regPath -Force
    Set-Content -Path "$LogPath\AutopilotWinUpdate.ps1.tag" -Value "Installed" -Force
}
